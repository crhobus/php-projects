...
class BookRest {
    ...
    public function run() {
        switch ($this->method) {
            case 'GET': $this->restGet();
                break;
            case 'POST': $this->restPost();
                break;
            case 'PUT': $this->restPut();
                break;
            case 'DELETE': $this->restDelete();
                break;
            default: $this->restError();
                break;
        }//switch
    }
    ...
    private function restPost() {
        ...
        try {
            ...
            if ($book != null) {
                ...
                $result['status'] = 'ERROR';
                $result['message'] = 'Livro adicionado mas não encontrado.';
            }
    ...
    private function restPut() {
        ...
        try {
            ...
            if ($book != null) {
                ...
                $result['status'] = 'ERROR';
                $result['message'] = 'Livro alterado mas não encontrado.';
            }
    ...
    private function restError() {
        try {
            throw new Exception("Operação inválida '$this->method'.");
        } catch (Excption $e) {
            $this->sendError($e);
        }
    }
